...
if (file_exists('/tmp/ABInfo_'.ABINFO.'.json')) {
    ...
    $txTarget = $abinfo['digital']['tg'];
    $dstarTune = isset($abinfo['last_tune']) ? strtoupper(trim($abinfo['last_tune'])) : '';
    if ($abinfo['tlv']['ambe_mode'] === 'DSTAR' && preg_match('/^(REF|XRF|DCS|XLX)[0-9A-Z]{3}[A-Z]L$/', $dstarTune)) {
        $txTarget = substr($dstarTune, 0, 6).' '.substr($dstarTune, 6, 1);
    }
    ...
    echo "<tr><th width=50%>Tx TG/Ref</th><td style=\"background: #f9f9f9;font-weight: bold;color:#ef7215;\">".$txTarget."</td></tr>\n";
